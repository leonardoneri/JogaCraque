...
import { Activity, AlertCircle, AlertTriangle, ArrowLeft, ChevronDown, ChevronUp, FastForward, Flag, Globe, MessageCircle, Play, Send, Share2, Shield, Swords, Trophy, Users, XCircle, XOctagon } from 'lucide-react';
import React, { useEffect, useRef, useState } from 'react';
import { useGame } from '../context/GameContext';
import { calculateTeamRating, generatePlayer, getCommentary, INITIAL_SQUAD_POSITIONS } from '../services/gameLogic';
import { socketService } from '../services/socketClient';
import { BallState, ChatMessage, MatchEvent, MatchEventType, MatchZone, Player, Position } from '../types';
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center pointer-events-none pb-48">
          <div className={`
           ...
           `}>
            ...
            <h1 className={`text-6xl font-display font-black italic uppercase drop-shadow-xl tracking-tighter mb-2 ${dramaticEvent.teamColor === 'green' ? 'text-green-500' : dramaticEvent.teamColor === 'red' ? 'text-red-500' : 'text-blue-500'
              }`}>
              {dramaticEvent.title}
            </h1>
              ...
              <div key={idx} className="flex items-center gap-3 p-2 hover:bg-white/5 rounded transition-colors border-b border-white/5 last:border-0">
                {player ? (
                  <>
                    ...
                    <span className={`text-xs font-bold px-2 py-0.5 rounded ${player.rating >= 90 ? 'bg-yellow-500/20 text-yellow-400' :
